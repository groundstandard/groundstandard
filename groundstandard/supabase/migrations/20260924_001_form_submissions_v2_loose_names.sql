-- A field can be called "First Name", first_name or FIRST-NAME in the builder;
-- to us it is the same field. The trigger now finds the person's details by
-- that loose name, so a form whose fields were typed the way a person types
-- still fills first_name, last_name, name, email and phone. Same function as
-- 004/005 with the lookups changed; safe to run again.

create or replace function fsv2_pick(p jsonb, wanted text)
returns text
language sql
immutable
as $$
  select value #>> '{}'
  from jsonb_each(p)
  where lower(regexp_replace(trim(key), '[ -]+', '_', 'g')) = wanted
    and nullif(trim(value #>> '{}'), '') is not null
  order by (key = wanted) desc
  limit 1
$$;
...
begin
  ...
  new.first_name := coalesce(new.first_name, fsv2_pick(p, 'first_name'), fsv2_pick(p, 'firstname'));
  new.last_name  := coalesce(new.last_name,  fsv2_pick(p, 'last_name'),  fsv2_pick(p, 'lastname'));
  new.name       := coalesce(new.name, fsv2_pick(p, 'name'),
                             nullif(trim(concat_ws(' ', new.first_name, new.last_name)), ''));
  new.email      := coalesce(new.email, lower(fsv2_pick(p, 'email')));
  new.phone      := coalesce(new.phone, fsv2_pick(p, 'phone'));
